// Seeds the Social Calendar with a starter set of Indian festivals and community
// functions. Safe to re-run: each event is only inserted if one with the same
// title doesn't already exist, so this won't create duplicates on a second run.
//
